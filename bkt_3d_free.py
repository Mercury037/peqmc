...
#     normalize   （新版/旧版都可，本脚本会做兼容）
#   model.py 里应提供：
#     PEMCNet     （建议是支持 use_theta 的新版；旧版仅能加载 use_theta=True checkpoint）
...
from simulation_qmc import *   # 你的函数都在这里
...
# 0) 兼容性辅助
# ============================================================
def _supports_init_kwarg(cls, kw: str) -> bool:
    try:
...
def _infer_use_theta_from_state_dict(state: dict) -> bool:
    """
    根据 checkpoint 参数名自动判断是不是 use_theta 模型。
    经验规则：
      - 若存在 theta_branch.* 参数 => use_theta=True
      - 若不存在 theta_branch.* 参数 => use_theta=False（theta-free）
    """
    return any(k.startswith("theta_branch.") for k in state.keys())


def _normalize_eval_auto(theta, X, norm: dict, use_theta: bool):
    """
    兼容两类 normalize:
      - 新版: normalize(theta, X, norm, use_theta=...)
      - 旧版: normalize(theta, X, norm)
    若 use_theta=False 且旧版 normalize 不支持该参数，则手动只标准化 X。
    """
    # 优先尝试新版签名
    try:
        return normalize(theta, X, norm, use_theta=use_theta)
    except TypeError:
        # 回退旧版 normalize(theta, X, norm)
        if use_theta:
            return normalize(theta, X, norm)
        else:
            # theta-free + 老版 normalize：手动只标准化 X
            X_n = (X - norm["X_mean"]) / norm["X_std"]
            return theta, X_n


def _net_forward_eval_auto(net, theta, X, use_theta: bool):
    """
    兼容:
      - use_theta=True  -> net(theta, X)
      - use_theta=False -> 优先 net(X)，失败则 net(theta, X)（兼容某些“忽略theta”的写法）
    """
    if use_theta:
        return net(theta, X)
...
def load_all(results_dir: str, device: str | None = None):
    ...
    # 先读 checkpoint，用于推断 use_theta
    state = torch.load(model_path, map_location=device)

    # 兼容 {"state_dict": ...} 保存格式（你当前大概率不是，但写上更稳）
    if isinstance(state, dict) and "state_dict" in state and isinstance(state["state_dict"], dict):
        state = state["state_dict"]

    if not isinstance(state, dict):
        raise TypeError(f"Unexpected checkpoint format at {model_path}, expected state_dict dict.")

    # use_theta：优先读 config；没有就从 checkpoint 推断
    if hasattr(cfg, "use_theta"):
        use_theta = bool(cfg.use_theta)
    else:
        use_theta = _infer_use_theta_from_state_dict(state)
        cfg.use_theta = use_theta

    # use_bn
    use_bn = getattr(cfg, "use_bn", False)

    # 打印一下当前导入的是哪个 model 类（排雷）
    try:
        print("[DEBUG] PEMCNet.__init__ =", inspect.signature(PEMCNet.__init__))
    except Exception:
        pass
    ...
    model_supports_use_theta = _supports_init_kwarg(PEMCNet, "use_theta")
    if model_supports_use_theta:
        init_kwargs["use_theta"] = use_theta
    else:
        # 如果 checkpoint 是 theta-free，但当前 model.py 的 PEMCNet 不支持 use_theta，就没法加载
        if not use_theta:
            raise RuntimeError(
                "当前导入的 PEMCNet 不支持 use_theta 参数，但 checkpoint 看起来是 theta-free 模型（use_theta=False）。\n"
                "请确认你导入的是新版 model.py（支持 use_theta 的 PEMCNet）。"
            )

    net = PEMCNet(**init_kwargs).to(device)
    ...
    try:
        ...
        msg = (
            f"\n加载 checkpoint 失败。\n"
            f"推断 use_theta={use_theta}, dimX={cfg.dimX}, use_bn={use_bn}\n"
            f"model_path={model_path}\n"
            f"这通常意味着：测试脚本实例化的模型结构和训练时不一致（比如 theta-free vs theta-model）。\n"
            f"原始错误:\n{e}"
        )
...
def predict_g_points_adaptive(
    ...
    use_theta = bool(getattr(cfg, "use_theta", True))

    if mode == "2d":
        theta_n, X_n = _normalize_eval_auto(theta_points, X_points, norm, use_theta=use_theta)
        g = _net_forward_eval_auto(net, theta_n, X_n, use_theta=use_theta).squeeze(-1)  # [N]
        return g
        ...
        theta_n, X_n = _normalize_eval_auto(theta_in, X_in, norm, use_theta=use_theta)
        g = _net_forward_eval_auto(net, theta_n, X_n, use_theta=use_theta)  # [1,N,1]
...
def main():
    ...
    print(f"[OK] use_theta    : {getattr(cfg, 'use_theta', None)}")
    print(f"[OK] Eval mode    : {_infer_eval_mode(norm, cfg)}")
    print(f"[OK] X_mean shape : {tuple(norm['X_mean'].shape)}")

    th_shape = tuple(norm["theta_mean"].shape) if "theta_mean" in norm else None
    print(f"[OK] th_mean shape: {th_shape}")
