...
#   3) 对 term1 再分解：
#      term1 = ybar - g1bar
#      Var(term1) = Var(ybar) + Var(g1bar) - 2Cov(ybar, g1bar)
#
# 依赖：
...
def load_all(results_dir: str, device: str | None = None):
    ...
    # 兼容你的不同版本 PEMCNet（有的有 use_bn，有的没有）
    try:
...
def pemc_mean_var(
    ...
    var_recon = (var_t1 + var_t2 + two_cov_t1_t2) if np.isfinite(var_t1) and np.isfinite(var_t2) and np.isfinite(two_cov_t1_t2) else float("nan")
    var_recon_err = (var_total - var_recon) if np.isfinite(var_total) and np.isfinite(var_recon) else float("nan")

    # term1 内部分解：t1 = ybar - g1bar
    ...
    minus2cov_ybar_g1bar = -2.0 * cov_ybar_g1bar if np.isfinite(cov_ybar_g1bar) else float("nan")
    var_t1_recon = (var_ybar + var_g1bar + minus2cov_ybar_g1bar) if np.isfinite(var_ybar) and np.isfinite(var_g1bar) and np.isfinite(minus2cov_ybar_g1bar) else float("nan")
    ...
    summary = {
        ...
        "minus2cov_ybar_g1bar": float(minus2cov_ybar_g1bar),
...
def main():
    ...
    parser.add_argument(
        "--results_dir",
        type=str,
        default=os.path.join(os.getcwd(), "results_lookback_Xdim34_N20_loss2"),
        help="训练结果目录"
    )
    parser.add_argument(
        "--device",
    ...
    for B in B_list:
        ...
        out, rep_df = pemc_mean_var(
            ...
            nD=args.nD, T=args.T, method=cfg.method,
            return_rep_df=True
        )
        ...
        rows_pemc.append({
            ...
            "minus2cov_ybar_g1bar": out["minus2cov_ybar_g1bar"],
    ...
    rows_base = []
    for B in B_list:
        rows_base.append(mc_mean_var(B, n_rep, cfg, theta_fixed, nD=args.nD, T=args.T, method=cfg.method))
        rows_base.append(qmc_mean_var(B, n_rep, cfg, theta_fixed, nD=args.nD, T=args.T, method=cfg.method))
    ...
    print("\n===== term1 内部分解（term1 = ybar - g1bar）=====")
    print(
        df_pemc[[
            "N1",
            "var_term1", "var_ybar", "var_g1bar", "minus2cov_ybar_g1bar",
            ...
            formatters={
                ...
                "var_g1bar": lambda x: f"{x:.12e}",
                "minus2cov_ybar_g1bar": lambda x: f"{x:.12e}",
